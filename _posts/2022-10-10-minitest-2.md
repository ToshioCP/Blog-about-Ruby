...
minitestバージョン6からモック（とスタブ）はminitest本体から分離・独立して、minitest-mockになりました。

- [minitest-mockのGitHubレポジトリ](https://github.com/minitest/minitest-mock)
- [minitest-mockのドキュメント](https://docs.seattlerb.org/minitest-mock/)

現時点（2026/2/18）でのRubyの最新版は4.0.1です。
minitest本体は、Ruby4.0.1のインストール時に（Gemとして）組み込まれていますが、minitest-mockは含まれません。
以下のようにインストールしてください。
...
また、テスト・スクリプトにも追加が必要です。

```ruby
require 'minitest/autorun'
require 'minitest/mock'    # <= 追加した部分
```
...
バックティック・メソッドは「[Kernelモジュール]({{ site.baseurl }}{% post_url 2022-9-19-kernel %})」を参照してください。
...
テスト本体ではrunメソッドの出力結果（上記のDBの各メソッドの出力のトータル）と期待される文字列を比較するだけです。
このテストの良いところは
