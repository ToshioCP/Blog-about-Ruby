...
- show: ひとつのモデルのデータを表示。クライアントはindexからモデルを指定してshowに先にすることが多い
- new: モデルの新規作成の画面
- create: new画面から送られたデータにより、モデルを新規作成するアクション
- edit: 既存のモデルのデータを変更するための画面。クライアントはshowからアクセスすることが多い
- update: edit画面から送られたでデータにより、モデルを更新するアクション
- destroy: 既存のモデルを消去するアクション。クライアントはshowからアクセスすることが多い

Railsでは、「RESTful」（レストフル）なウェブアクセスでは、アクションをこの7つにまとめることができる、としています。
RESTfulは形容詞で「RESTな」ということです。
では、REATとは何かというと、それは「Representational State Transfer」を短くしたもので、ウェブ・インターフェースのアーキテクチャスタイルです。
...
- Railsはこのパターンでのプログラムに便利な様々な工夫をしている
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item">
          <% if action_name == "show" %>
            <%= link_to "削除", word_path(@word), style: "text-decoration: none;", class: "nav-link", data: {turbo_method: :delete, turbo_confirm: "Are you sure?"} %>
          <% else %>
...
前回のwordbookではフォームの送信ボタンが削除のきっかけになっていましたが、今回はナビゲーションバーの「削除」ボタンがきっかけになっています。
...
<% if @words %>
  <table class="table table-bordered">
    ...
    <% @words.each do |word| %>
      <tr>
        <td><%= link_to word[:en], word_path(word) %></td>
        <td><%= word[:jp] %></td>
      </tr>
    <% end %>
    </tbody>
...
`word_path(word)`がshowアクションへのリンクです。
...
ビュー`/app/views/words/show.html.erb`は次のように成ります。
...
実は`word_path(@word)`と今までやっていたのが`@word`だけでも同じようにリンク先を生成してくれるのです。
これがリソースフル・ルーティングのさらなる恩恵です。
...
前回のWordbookと比べると大きな差はないものの、リソースフル・ルーティングを使ったおかげで楽をした部分がありました。
